...
from lib.core.config import AnimateLy_DB_DIR, AnimateLy_DATA_DIR
...
def read_data(folder, set, debug=False):
    ...
    if set == 'test' or set == 'validation':
        J_regressor = torch.from_numpy(np.load(osp.join(AnimateLy_DATA_DIR, 'J_regressor_h36m.npy'))).float()
...
if __name__ == '__main__':
    ...
    joblib.dump(dataset, osp.join(AnimateLy_DB_DIR, '3dpw_val_db.pt'))

    dataset = read_data(args.dir, 'test', debug=debug)
    joblib.dump(dataset, osp.join(AnimateLy_DB_DIR, '3dpw_test_db.pt'))
