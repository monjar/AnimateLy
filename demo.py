...
from lib.models.animately import AnimateLy_Demo
...
def main(args):
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        dataloader = DataLoader(dataset, batch_size=args.animately_batch_size, num_workers=16)
        ...
        animately_results[person_id] = output_dict

    del model

    end = time.time()
    fps = num_frames / (end - animately_time)

    print(f'AnimateLy FPS: {fps:.2f}')
    total_time = time.time() - total_time
    print(f'Total time spent: {total_time:.2f} seconds (including model loading time).')
    print(f'Total FPS (including model loading time): {num_frames / total_time:.2f}.')

    print(f'Saving output results to \"{os.path.join(output_path, "animately_output.pkl")}\".')

    joblib.dump(animately_results, os.path.join(output_path, "animately_output.pkl"))
    ...
    if not args.no_render:
        ...
        frame_results = prepare_rendering_results(animately_results, num_frames)
        mesh_color = {k: colorsys.hsv_to_rgb(np.random.rand(), 0.5, 1.0) for k in animately_results.keys()}
        ...
        save_name = f'{vid_name.replace(".mp4", "")}_animately_result.mp4'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--animately_batch_size', type=int, default=450,
                        help='batch size of AnimateLy')
